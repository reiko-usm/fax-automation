...
HEADERS = ["FAX日付", "商品名", "数量", "発注者", "納品先名称", "納品先郵便番号", "納品先住所", "納品希望日", "発注№", "お客様№"]

SYSTEM_PROMPT = """あなたはFAX発注書を読み取るアシスタントです。
画像に含まれる発注書を全て読み取り、必ずJSON配列形式のみで返してください。
発注書が1件でも配列で返してください。説明文は不要です。

FAX日付：用紙の右端・左端・ヘッダー部分の送信日時・作成日・発注日・FAXシステムの日付を読み取ること。
納品先は名称・郵便番号・住所に分けて抽出すること。
...
if "results" in st.session_state and st.session_state["results"]:
    ...
    display_cols = ["FAX日付", "商品名", "数量", "発注者", "納品先名称",
