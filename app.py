...
HEADERS = ["FAX日付", "商品名", "数量", "単位", "発注者", "納品先名称", "納品先郵便番号", "納品先住所", "納品希望日", "発注№", "お客様№"]

SYSTEM_PROMPT = """あなたはFAX発注書を読み取るアシスタントです。
画像に含まれる発注書を全て読み取り、必ずJSON配列形式のみで返してください。
発注書が1件でも配列で返してください。説明文は不要です。
項目が記載されていない場合は空文字列にすること。

【商品名の正規化】
以下の正式商品名リストに照合し、最も近いものに統一してください。
表記ゆれ・略称・別名があっても正式名称に変換すること：
- ライフ
- 万代
- 大入り45g
- 3本きなこ棒
...
- 棒キナコ 24本×16×10
- 当たり無し
リストに該当しない商品はそのまま記載すること。

【数量と単位】
数量は数字のみで返すこと（例：133）。
単位は別フィールドに返すこと（例：本、個、箱、ケース）。

【FAX日付】
用紙の右端・左端・ヘッダー部分の送信日時・作成日・発注日・FAXシステムの日付を読み取ること。

返却形式：
[
  {
    "fax日付": "YYYY年M月D日形式、不明は空",
    "商品名": "正規化された正式商品名",
    "数量": "数字のみ（例：133）",
    "単位": "単位のみ（例：本、個、箱）",
    "発注者": "発注者名または会社名",
    "納品先名称": "納品先の会社名・店舗名",
    "納品先郵便番号": "例：123-4567",
    "納品先住所": "都道府県から番地・建物名まで",
    "納品希望日": "YYYY年M月D日形式、不明は空",
    "発注番号": "発注番号・注文番号",
    "顧客番号": "お客様番号・顧客番号"
  }
]"""
...
def process_file(uploaded_file):
    """アップロードファイルをページごとのPNG画像リストに変換する。"""
    ...
    if filename.endswith(".pdf"):
        ...
        pages.append((file_bytes, "image/jpeg"))
    else:
        pages.append((file_bytes, "image/png"))

    return pages


def extract_fax_data(image_bytes, media_type):
    """Claude Vision APIで発注書を抽出する（1ページに複数発注書対応）。"""
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "この画像に含まれる発注書を全て読み取り、JSON配列で返してください。"},
                ],
            }
        ],
    )
...
if uploaded_files:
    ...
    if st.button("🔍 AIで一括読み取り", type="primary", use_container_width=True):
        ...
        for i, f in enumerate(uploaded_files):
            with st.spinner(f"読み取り中: {f.name}（{i+1}/{total}）"):
                try:
                    f.seek(0)
                    pages = process_file(f)
                    for page_num, (image_bytes, media_type) in enumerate(pages):
                        orders = extract_fax_data(image_bytes, media_type)
                        label = f.name if len(pages) == 1 else f"{f.name}（{page_num+1}ページ）"
                        for order in orders:
                            results.append({
                                "FAX日付": order.get("fax日付", ""),
                                "商品名": order.get("商品名", ""),
                                "数量": order.get("数量", ""),
                                "単位": order.get("単位", ""),
                                "発注者": order.get("発注者", ""),
                                "納品先名称": order.get("納品先名称", ""),
                                "納品先郵便番号": order.get("納品先郵便番号", ""),
                                "納品先住所": order.get("納品先住所", ""),
                                "納品希望日": order.get("納品希望日", ""),
                                "発注№": order.get("発注番号", ""),
                                "お客様№": order.get("顧客番号", ""),
                                "_label": label,
                            })
...
if "results" in st.session_state and st.session_state["results"]:
    ...
    display_cols = ["FAX日付", "商品名", "数量", "単位", "発注者", "納品先名称",
                    "納品先郵便番号", "納品先住所", "納品希望日", "発注№", "お客様№"]

    edited_df = st.data_editor(
        pd.DataFrame(st.session_state["results"])[display_cols + ["_label"]],
        column_config={"_label": st.column_config.TextColumn("ファイル名", disabled=True)},
    ...
    if st.button("✅ 全て保存", type="primary", use_container_width=True):
        ...
        for _, row in edited_df.iterrows():
            ...
            success, result = save_to_sheets(data)
            if success:
                success_count += 1
                last_url = result
                st.success(f"✅ {row.get('_label', '')}")
            else:
                st.error(f"❌ {row.get('_label', '')}: {result}")

        if success_count > 0:
            st.balloons()
            st.markdown(f"[📊 Google Sheetsを開く]({last_url})")
